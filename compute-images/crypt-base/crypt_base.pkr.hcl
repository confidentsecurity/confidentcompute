/*
  ...
  Variables:
    ...
    - CONFSEC_EPHEMERAL_INTEGRITY: Whether or not dm-integrity is enabled for the ephemeral partitions
    - CONFSEC_NO_SERIAL: Whether or not to disable the cloud serial console
...
variable "ephemeral_integrity" {
  type    = bool
  default = env("CONFSEC_EPHEMERAL_INTEGRITY") != "" ? env("CONFSEC_EPHEMERAL_INTEGRITY") == "true" : false
}
...
variable "no_serial" {
  type    = bool
  default = env("CONFSEC_NO_SERIAL") != "" ? env("CONFSEC_NO_SERIAL") == "true" : false
}
...
variable "aux_source_image" {
  ...
  validation {
    condition     = var.aux_source_image == "" || can(regex(".*build-aux-.+$", var.aux_source_image))
...
variable "image_family" {
  type    = string
  default = env("CONFSEC_IMAGE_FAMILY") != "" ? env("CONFSEC_IMAGE_FAMILY") : "crypt-base"
}
...
local "azure_vm_size" {
  # Use a larger instance size for this build, since its the slowest of them all.
  expression = var.disk_interface == "NVME" ? "Standard_D8s_v6" : "Standard_D8s_v4"
}

# Shared Build Locals
local "aux_source_image_family" {
  expression = "build-aux"
}
local "build_id" {
  expression = uuidv4()
}
...
source "googlecompute" "default" {
  ...
  image_labels = {
    ...
    "ephemeral_integrity" = var.ephemeral_integrity
    "verity_panic"        = var.verity_panic
    "optimize_disk"       = var.optimize_disk
  }
  use_iap             = var.use_iap
  project_id          = var.gcp_project_id
  source_image_family = var.host_source_image_family
...
source "azure-arm" "default" {
  ...
  # Available option thanks to https://github.com/confidentsecurity/packer-plugin-azure/pull/1
  # Important we set this configuration at VM creation time to avoid async propagation delays.
...
build {
  ...
  provisioner "shell" {
    ...
    execute_command = <<EOF
      ...
      CLOUD=${local.build_args.cloud_env_var[source.type]}                             \
      sudo -E bash '{{.Path}}'
    EOF
  }
  ...
  post-processors {
    post-processor "shell-local" {
      only = ["qemu.default"]
      inline = [
        # We've attached auxiliary image manually, so we have to move it to the output directory manually.
