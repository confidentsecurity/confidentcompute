...
variable "qemu_artifact_registry_source_project_id" {
  type      = string
  default   = "<empty>"
  sensitive = true
}
...
variable "qemu_artifact_registry_aux_source_artifact_name" {
  type    = string
  default = "crypt-base:0.0.0:<empty>"
  validation {
    condition     = can(regex("^[^:]+:[^:]+:[^:]+$", var.qemu_artifact_registry_aux_source_artifact_name))
    error_message = "The artifact name must match the pattern '<package>:<version>:<name>', e.g., crypt-base:2025.1014.153721:crypt-base.img."
  }
  validation {
    condition     = can(regex("^crypt-base:.*", var.qemu_artifact_registry_aux_source_artifact_name))
    error_message = "The package name of aux source artifact name should be 'crypt-base'."
  }
}
...
variable "qemu_source_primary_img_path" {
  ...
  default     = "/tmp/compute-base-primary.img"
}
...
variable "qemu_source_aux_img_path" {
  ...
  default     = "/tmp/compute-base-aux.img"
}

variable "qemu_artifact_registry_target_package" {
  type    = string
  default = "compute-base"
}
...
variable "qemu_compress_result" {
  type        = bool
  default     = false
  description = "Compress the result image, takes 13 mins, reduces size from 33GBs to 4.5GBs."
}

### Locals

data "qemu-sshkey" "qemu_install_key" {
  name = "compute-base-key-pair-${timestamp()}"
}

locals {
  install_meta_data = <<-EOF
    instance-id: compute-base-${timestamp()}
    local-hostname: compute-base
    EOF
...
data "external-raw" "download-primary-img" {
  program = ["../scripts/qemu_download_image_from_repository.sh", var.qemu_artifact_registry_source_project_id, var.qemu_artifact_registry_location, var.qemu_artifact_registry_repository_name, var.qemu_artifact_registry_primary_source_artifact_name, var.qemu_source_primary_img_path, var.qemu_local_only]
...
data "external-raw" "download-aux-img" {
  program = ["../scripts/qemu_download_image_from_repository.sh", var.qemu_artifact_registry_source_project_id, var.qemu_artifact_registry_location, var.qemu_artifact_registry_repository_name, var.qemu_artifact_registry_aux_source_artifact_name, var.qemu_source_aux_img_path, var.qemu_local_only]
  # qemu_download_image_from_repository returns a path where downloaded image is stored. Use it to create explicit dependency.
}
...
data "external-raw" "prepare-aux-img" {
  program = ["../scripts/qemu_prepare_aux_input.sh", "compute-base", data.external-raw.download-aux-img.result, "aux-ubuntu.img", "${var.aux_disk_size}G"]
}
...
locals {
  ...
  aux_output_image_basename = "aux-${local.qemu_image_name}"
  aux_output_image_path     = "${local.qemu_output_directory}/${local.aux_output_image_basename}"
}
...
source "qemu" "default" {
  ...
  skip_compaction  = true # skip optimizing output image because we only care about aux image.
